 ...
 * @param {number} limit max number of mails to fetch
 ...
 * @memberof ImapSimple
 */
ImapSimple.prototype.search = function (searchCriteria, fetchOptions, limit, callback) {
    var self = this;
    ...
    if (callback) {
        return nodeify(this.search(searchCriteria, fetchOptions, limit), callback);
    }
    ...
    return new Promise(function (resolve, reject) {
        ...
        self.imap.search(searchCriteria, function (err, uids) {
            ...
            console.log(`found ${uids.length} mails, begin fetch`);
            const uidsToFetch = uids.slice(0, limit);

            var fetch = self.imap.fetch(uidsToFetch, fetchOptions);
